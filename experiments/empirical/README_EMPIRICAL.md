# Empirical Validation of Mode Averaging (MAP) on Commercial Models

This directory contains the experiment pipeline for testing the Mode Averaging Principle (MAP) predictions on real commercial image generation models (OpenAI DALL-E 3 and Google Imagen 4).
...
3. Click "Create new secret key" — give it a name like "MAP experiment"
...
│   ├── openai/{category}/
│   └── google/{category}/
├── embeddings/                 ← precomputed embeddings (tracked in git)
│   ├── embeddings.npz
│   ├── embeddings_dinov2.npz
│   └── (tracked in git — the analysis reproduces from these)

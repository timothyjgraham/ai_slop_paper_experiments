#!/usr/bin/env python3
"""
Empirical Analysis Pipeline for Mode-Averaging (MAP) Validation
=====================================================
...
def diversity_index(samples):
    """D(P) = exp(H / k): per-dimension normalised Gaussian-fit entropy,
    computed on the non-degenerate spectrum of the sample covariance.

    With n samples in d dimensions (n << d), the sample covariance has
    rank at most n-1; eigenvalues below that rank are numerical noise
    whose magnitude depends on the linear-algebra backend (BLAS/LAPACK
    build), which previously made the log-determinant - and hence D -
    irreproducible across machines. We therefore restrict the entropy
    to the top k = min(n-1, d) eigenvalues, additionally discarding any
    eigenvalue below a *relative* floor (lambda_max * 1e-10), and
    normalise per retained dimension. This makes D deterministic across
    platforms and comparable across embedding spaces of different
    dimension. Corresponds to Definition 2.1 in the paper.
    """
    n, d = samples.shape
    if n < 2:
        return 0.0
    ...
    k = min(n - 1, d)
    eigvals = eigvals[:k]
    eigvals = eigvals[eigvals > eigvals[0] * 1e-10]
    if len(eigvals) == 0:
        return 0.0
    k = len(eigvals)
    logdet = float(np.sum(np.log(eigvals)))
    entropy = 0.5 * k * (1 + np.log(2 * np.pi)) + 0.5 * logdet
    return float(np.exp(entropy / k))  # per-retained-dimension normalisation
...
def significance_tests(a2_results):
    ...
    # Effect size: Cohen's d with the standard size-weighted pooled SD
    # (sample variances, ddof=1), plus Hedges' g small-sample correction.
    n_a, n_b = len(majority_r), len(minority_r)
    var_a = np.var(majority_r, ddof=1)
    var_b = np.var(minority_r, ddof=1)
    ...
    results["hedges_g"] = float(hedges_g)
    print(f"\n  Cohen's d (pooled SD): {cohens_d:.3f};  Hedges' g: {hedges_g:.3f}")
...
def compile_summary(a1_results, a2_results, a3_results, figures_dir):
    ...
    print("\n" + "=" * 60)
    print("  MAP PREDICTION SUMMARY")
    print("=" * 60)

    summary = {
        "experiment": "Empirical validation of mode averaging (MAP) on commercial models",
...
def main():
    parser = argparse.ArgumentParser(
        description="Empirical analysis pipeline for mode-averaging (MAP) validation"
    )
    parser.add_argument(
